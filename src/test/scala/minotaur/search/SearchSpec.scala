...
import minotaur.model.{Search,Location,North,South}
...
class SearchSpec extends Specification {
  ...
  def blackNorth(search: Search) = {
    search.findPath(board, board.pawnLocation(Black), North) === Some(Seq(
  ...
  def whiteSouth(search: Search) = {
    search.findPath(board, board.pawnLocation(White), South) === Some(Seq(
